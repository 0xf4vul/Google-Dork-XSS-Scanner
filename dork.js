...
██████╔╝╚█████╔╝██║░░██║██║░╚██╗███████╗██║░░██║
╚═════╝░░╚════╝░╚═╝░░╚═╝╚═╝░░╚═╝╚══════╝╚═╝░░╚═╝
      Powered By : FDCI CYBER ART
      

`);
const request = require('request');
const inidork = 'inurl:index.php?id='
var DORK = 'id=';
...
var options = {
    url: 'https://www.google.com/search?q='+encodeURI(inidork) +'&num=100',
    headers: headers
};
...
function callback(error, response, body) {
	...
	console.log(spliter.length, ' Terjadi Kesalahan')
...
function testaXss(url) {
	request({url}, function (error, response, body) {  
		if(!error && body.search(DORKXSS) != -1) console.log(url, ' <= You Got Vuln XSS');
